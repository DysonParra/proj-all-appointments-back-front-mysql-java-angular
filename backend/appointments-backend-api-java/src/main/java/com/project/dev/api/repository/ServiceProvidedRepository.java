/*
 * @fileoverview    {ServiceProvidedRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code ServiceProvidedRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface ServiceProvidedRepository extends JpaRepository<ServiceProvided, Long> {
     ...
     */
    public List<ServiceProvided> findByIntId(Long id);
    ...
    @Query("SELECT m FROM ServiceProvided m WHERE m.intId LIKE CONCAT('%', :strSearch, '%')")
    public List<ServiceProvided> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM ServiceProvided m WHERE m.intId LIKE CONCAT('%', :strSearch, '%')")
    public Page<ServiceProvided> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
