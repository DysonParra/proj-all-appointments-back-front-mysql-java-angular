/*
 * @fileoverview    {ServiceBookedRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code ServiceBookedRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface ServiceBookedRepository extends JpaRepository<ServiceBooked, Long> {
     ...
     */
    public List<ServiceBooked> findByIntId(Long id);
    ...
    @Query("SELECT m FROM ServiceBooked m WHERE m.intId LIKE CONCAT('%', :strSearch, '%')")
    public List<ServiceBooked> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM ServiceBooked m WHERE m.intId LIKE CONCAT('%', :strSearch, '%')")
    public Page<ServiceBooked> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
