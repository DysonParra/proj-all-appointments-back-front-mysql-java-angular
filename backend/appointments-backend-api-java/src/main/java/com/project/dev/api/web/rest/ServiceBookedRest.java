/*
 * @fileoverview    {ServiceBookedRest} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
import com.project.dev.api.web.rest.assembler.ServiceBookedRestAssembler;
...
@Api(value = "ServiceBooked API")
public class ServiceBookedRest {

    private final Logger log = LoggerFactory.getLogger(ServiceBookedRest.class);
    private final ServiceBookedService entityService;
    private final ServiceBookedRestAssembler entityRestAssembler;
     ...
     */
    public ServiceBookedRest(ServiceBookedService entityService,
            ServiceBookedRestAssembler entityRestAssembler) {
    ...
    public CollectionModel<EntityModel<ServiceBookedDTO>> getAllEntities() {
        ...
        try {
            ...
            CollectionModel<EntityModel<ServiceBookedDTO>> entitiesCollection = new CollectionModel<>(entities);
            entitiesCollection.add(linkTo(methodOn(ServiceBookedRest.class).getAllEntities()).withSelfRel());
    ...
    public ResponseEntity<List<ServiceBookedDTO>> getAllEntitiesPaged(Pageable pageable) {
    ...
    @GetMapping("/ServiceBooked/{query}/pages")
    public ResponseEntity<List<ServiceBookedDTO>> searchEntitiesPaged(@PathVariable String query, Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/ServiceBooked/{query}/pages/" + query);
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
